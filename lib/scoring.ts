...
export function computeScoring(answers: Answers): ScoringResult {
  ...
  if (capacity.maxByIncome >= 400_000) {
    add(8, "Le revenu du ménage soutient un budget confortable pour l'Estrie", "positive");
  } else if (capacity.maxByIncome >= 250_000) {
    add(3, "Le revenu du ménage soutient un budget réaliste en Estrie", "neutral");
