...
function summaryFor(verdict: Verdict, answers: Answers, scoring: ScoringResult): string {
  ...
  switch (verdict) {
    ...
    case "a_batir":
      return `Aujourd'hui, les chiffres ne soutiennent pas encore un achat en Estrie — et le savoir maintenant t'évite de perdre du temps. En travaillant le revenu retenu et la mise de fonds, ton projet devient réaliste plus vite que tu penses.`;
  }
}
    ...
    case "pret":
      return [
        ...
        },
        {
          ...
          description: "Conditions, inspection, délais : ce qui fait accepter une offre en Estrie n'est pas toujours le prix le plus élevé.",
        },
      ];
    case "financement":
      return [
...
function marketInsightFor(answers: Answers): string {
  const secteurs = regionNames(answers);
  const secteur = secteurs[0] ?? "l'Estrie";
